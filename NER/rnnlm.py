...
class RNNLM(NNBase):
    ...
    def _acc_grads(self, xs, ys):
        ...
        for t in reversed(xrange(ns)):
            y = make_onehot(ys[t], len(ps[t]))
            delta = ps[t] - y
            self.grads.U += outer(delta,hs[t])
            self.grads.H += outer(delta.dot(self.params.U)*sigmoid_grad(hs[t]),hs[t-1])
            delta2 = delta.dot(self.params.U)*sigmoid_grad(hs[t])
            self.sgrads.L[xs[t]] = delta2

            for s in xrange(min(self.bptt-1,t)):
                delta2_new = delta2.dot(self.params.H)*sigmoid_grad(hs[t-s-1])
                self.grads.H += outer(delta2_new,hs[t-s-2])
                self.sgrads.L[xs[t-s-1]] = delta2_new
                delta2 = delta2_new
